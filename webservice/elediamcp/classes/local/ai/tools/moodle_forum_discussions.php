    ...
    private static function list_discussions(array $cmids, stdClass $user, int $limit, int $offset): array {
        ...
        foreach ($cmids as $cmid) {
            ...
            foreach ($discussions as $discussion) {
                ...
                // Ready-made citation (see class source), so a discussion quoted
                // in an answer is linkable like any other course material.
                $entry['source'] = source::build(
                    $entry['subject'],
                    $entry['url'],
                    (string) ($firstpost->message ?? ''),
                    (int) $cm->id,
                    // From the module, not an outer variable: this loop also runs
                    // in the across-all-courses branch, which has no course id.
                    (int) $cm->course
                );
                $entries[] = $entry;
            }
