...
class configuration_form extends moodleform {
    ...
    protected function definition(): void {
        ...
        $mform->addElement('html', \html_writer::tag(
            'h3',
            get_string('configuration_oauth_heading', 'webservice_elediamcp'),
            ['class' => 'webservice-elediamcp-card__title']
        ));
        $mform->addElement('html', \html_writer::div(
            get_string('setting_oauth_warning', 'webservice_elediamcp'),
            'webservice-elediamcp-section-warning alert alert-warning'
        ));
        $mform->addElement('html', \html_writer::start_tag('section', [
            'class' => 'webservice-elediamcp-card',
        ...
        $mform->addElement('html', \html_writer::span('', '', ['id' => 'admin-oauth_enabled']));
        $mform->addElement(
            'advcheckbox',
            'oauth_enabled',
            get_string('setting_oauth_enabled', 'webservice_elediamcp'),
            get_string('setting_oauth_enabled_desc', 'webservice_elediamcp'),
            null,
            [0, 1]
        );
        $mform->addHelpButton('oauth_enabled', 'setting_oauth_enabled', 'webservice_elediamcp');

        $mform->addElement('html', \html_writer::span('', '', ['id' => 'admin-oauth_allow_dynamic_registration']));
        $mform->addElement(
            'advcheckbox',
            'oauth_allow_dynamic_registration',
            get_string('setting_oauth_allow_dynamic_registration', 'webservice_elediamcp'),
            get_string('setting_oauth_allow_dynamic_registration_desc', 'webservice_elediamcp'),
            null,
            [0, 1]
        );
        $mform->addHelpButton(
            'oauth_allow_dynamic_registration',
            'setting_oauth_allow_dynamic_registration',
            'webservice_elediamcp'
        );
        $mform->disabledIf('oauth_allow_dynamic_registration', 'oauth_enabled', 'notchecked');

        $mform->addElement('html', \html_writer::span('', '', ['id' => 'admin-oauth_code_ttl']));
        $mform->addElement(
            'text',
            'oauth_code_ttl',
            get_string('setting_oauth_code_ttl', 'webservice_elediamcp'),
            ['size' => 8]
        );
        $mform->setType('oauth_code_ttl', PARAM_INT);
        $mform->addHelpButton('oauth_code_ttl', 'setting_oauth_code_ttl', 'webservice_elediamcp');
        $mform->addElement(
            'static',
            'oauth_code_ttl_desc',
            '',
            get_string('setting_oauth_code_ttl_desc', 'webservice_elediamcp')
        );

        $mform->addElement('html', \html_writer::span('', '', ['id' => 'admin-oauth_token_ttl']));
        $mform->addElement(
            'text',
            'oauth_token_ttl',
            get_string('setting_oauth_token_ttl', 'webservice_elediamcp'),
            ['size' => 8]
        );
        $mform->setType('oauth_token_ttl', PARAM_INT);
        $mform->addHelpButton('oauth_token_ttl', 'setting_oauth_token_ttl', 'webservice_elediamcp');
        $mform->addElement(
            'static',
            'oauth_token_ttl_desc',
            '',
            get_string('setting_oauth_token_ttl_desc', 'webservice_elediamcp')
        );
    ...
    public function validation($data, $files): array {
        ...
        foreach (
            ['token_retention_days', 'rate_limit_per_minute', 'rate_limit_per_hour',
                'max_request_size', 'tools_page_size', 'oauth_code_ttl', 'oauth_token_ttl'] as $field
        ) {
