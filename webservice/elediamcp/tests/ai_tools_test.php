...
final class ai_tools_test extends advanced_testcase {
    ...
    /**
     * Contributed tools flow through the registry: valid ones are appended and
     * resolvable, name collisions lose against built-ins, malformed names are
     * rejected with a developer debugging message.
     */
    public function test_contributed_tools_provider_hook(): void {
        global $CFG;
        require_once($CFG->dirroot . '/webservice/elediamcp/tests/fixtures/contributed_tool_fixture.php');
        // Fixtures are not autoloaded, so each class needs its own require.
        require_once($CFG->dirroot . '/webservice/elediamcp/tests/fixtures/colliding_tool_fixture.php');
        require_once($CFG->dirroot . '/webservice/elediamcp/tests/fixtures/malformed_tool_fixture.php');

        registry::set_test_tools([
            \webservice_elediamcp\tests\fixtures\contributed_tool_fixture::class,
            \webservice_elediamcp\tests\fixtures\colliding_tool_fixture::class,
            \webservice_elediamcp\tests\fixtures\malformed_tool_fixture::class,
        ]);
        try {
            $names = registry::names();

            $this->assertContains('fixture_echo', $names);
            $this->assertSame(
                \webservice_elediamcp\tests\fixtures\contributed_tool_fixture::class,
                registry::find('fixture_echo')
            );
            $this->assertTrue(registry::is_contributed('fixture_echo'));

            // The colliding name keeps resolving to the built-in tool.
            $this->assertSame(moodle_me::class, registry::find('moodle_me'));
            $this->assertSame(1, count(array_keys($names, 'moodle_me', true)));

            // The malformed name never enters the catalogue (one debugging
            // message from validation; the collision itself is silent).
            $this->assertNotContains('Fixture-Bad-Name', $names);
    ...
    /**
     * moodle_enrol_user rejects roles the caller may not assign (no escalation).
     */
    public function test_moodle_enrol_user_rejects_unassignable_role(): void {
        $this->resetAfterTest(true);
        ...
        try {
            ...
            $this->fail('Expected tool_exception for a role the teacher may not assign.');
        } catch (tool_exception $e) {
            $this->assertStringContainsString('not allowed to assign', $e->getMessage());
        }
        $this->assertFalse($this->is_user_enrolled_in_course((int) $target->id, (int) $course->id));
    }
