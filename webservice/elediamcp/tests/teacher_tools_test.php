...
final class teacher_tools_test extends advanced_testcase {
    ...
    /**
     * read_submission rejects a user_id that is not a participant of the assignment.
     */
    public function test_read_submission_rejects_non_participant(): void {
        $this->resetAfterTest(true);
        ...
        moodle_read_submission::execute(['cmid' => (int) $cm->id, 'user_id' => (int) $outsider->id], $teacher);
    }

    /**
     * grade_submission rejects a user_id that is not a participant of the assignment.
     */
    public function test_grade_submission_rejects_non_participant(): void {
        global $DB;
        $this->resetAfterTest(true);
        [, $teacher, , , $cm] = $this->grading_fixture();
        $outsider = $this->getDataGenerator()->create_user();
        ...
        try {
            moodle_grade_submission::execute([
                'cmid' => (int) $cm->id, 'user_id' => (int) $outsider->id, 'grade' => 50, 'confirm' => true,
            ], $teacher);
            $this->fail('Expected tool_exception for a non-participant user_id.');
        } catch (tool_exception $e) {
            $this->assertStringContainsString('not a participant', $e->getMessage());
        }
        $this->assertFalse($DB->record_exists('assign_grades', ['userid' => $outsider->id]));
    }

    /**
     * Blind-marking assignments are rejected by both grading-loop tools.
     */
    public function test_grading_loop_rejects_blind_marking(): void {
        $this->resetAfterTest(true);
        ...
        try {
            moodle_read_submission::execute(['cmid' => (int) $cm->id, 'user_id' => (int) $student->id], $teacher);
            $this->fail('Expected tool_exception for a blind-marking assignment (read).');
        } catch (tool_exception $e) {
            $this->assertStringContainsString('blind marking', $e->getMessage());
        }

        try {
            moodle_grade_submission::execute([
                'cmid' => (int) $cm->id, 'user_id' => (int) $student->id, 'grade' => 50, 'confirm' => true,
            ], $teacher);
            $this->fail('Expected tool_exception for a blind-marking assignment (grade).');
        } catch (tool_exception $e) {
            $this->assertStringContainsString('blind marking', $e->getMessage());
        }
    }
    ...
    /**
     * With separate groups and no accessallgroups a foreign group is out of reach.
     */
    public function test_read_submission_respects_separate_groups(): void {
        global $DB;
        $this->resetAfterTest(true);
