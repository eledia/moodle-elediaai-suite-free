...
final class server_test extends advanced_testcase {
    /**
     * Test initialize advertises prompts without listChanged.
     */
    public function test_initialize_advertises_prompts(): void {
        $this->resetAfterTest(true);
        $server = new server(WEBSERVICE_AUTHMETHOD_PERMANENT_TOKEN);
        $reflection = new ReflectionClass($server);
        $property = $reflection->getProperty('mcprequest');
        $property->setAccessible(true);
        $mcprequest = new request(['jsonrpc' => '2.0', 'method' => 'initialize', 'id' => 1, 'params' => []]);
        $property->setValue($server, $mcprequest);
        ...
        $this->assertSame([], $response['result']['capabilities']['prompts']);
    }

    /**
     * Test prompts/get returns invalid params for an unknown name.
     */
    public function test_prompts_get_unknown_name(): void {
        $this->resetAfterTest(true);
        $server = new server(WEBSERVICE_AUTHMETHOD_PERMANENT_TOKEN);
        $reflection = new ReflectionClass($server);
        $property = $reflection->getProperty('mcprequest');
        $property->setAccessible(true);
        $mcprequest = new request(['jsonrpc' => '2.0', 'method' => 'prompts/get', 'id' => 2,
            'params' => ['name' => 'not_a_prompt']]);
        $property->setValue($server, $mcprequest);
        $method = $reflection->getMethod('send_prompts_get_response');
        $method->setAccessible(true);
        ob_start();
        $method->invoke($server);
        $response = json_decode(ob_get_clean(), true);
    ...
    public function test_server_constants(): void {
        ...
        // Advertised server version is derived from $plugin->release so it can
        // never drift from the plugin release (see docs/04-tasks.md q01).
        $serverversion = server::get_server_version();
        ...
        $this->assertSame(
            $plugin->release,
            $serverversion,
            'Advertised serverInfo.version must equal $plugin->release from version.php.'
        );
