...
/**
 * AI-native question generation tool wrapping local_elediaai_questiongen.
 *
 * Only registered when local_elediaai_questiongen is installed (see
...
class moodle_generate_questions implements ai_tool {
    ...
    public static function description(): string {
        return 'Generates quiz questions and imports them into a question bank of the course. '
            ...
            . 'course activity via source_cmid). Requires local/elediaai_questiongen:use and '
    ...
    public static function execute(array $arguments, stdClass $user): array {
        ...
        if (!class_exists('\local_elediaai_questiongen\local\generator')) {
            throw new tool_exception('local_elediaai_questiongen is not installed on this site.');
        }
        ...
        if ($qbankaction === 'create') {
            ...
            require_capability('local/elediaai_questiongen:use', $coursecontext, $user->id);
            require_capability('moodle/question:add', $coursecontext, $user->id);
        }
        ...
        try {
            \local_elediaai_questiongen\local\generator::require_text_provider();
        ...
        if ($qbankcm === null) {
            ...
            self::require_bank_capabilities($qbankcm, $user);
        }
        $modcontext = context_module::instance($qbankcm->id);
        if ($categoryid <= 0) {
        ...
        try {
            $result = \local_elediaai_questiongen\local\generator::generate((object) [
                'mode' => $mode,
                'topic' => $topic,
    ...
    private static function resolve_qbank(stdClass $course, int $qbankcmid, stdClass $user): array {
        ...
        foreach ($banks as $cm) {
            ...
            $canuse = has_capability('local/elediaai_questiongen:use', $modcontext, $user->id)
        ...
        if (!empty($banks)) {
            throw new tool_exception(
                'You do not have permission to add questions to any question bank in this course. '
                . 'Ask for access to an existing bank instead of creating a new one.',
    ...
    private static function require_bank_capabilities(cm_info $cm, stdClass $user): void {
        $modcontext = context_module::instance($cm->id);
        require_capability('local/elediaai_questiongen:use', $modcontext, $user->id);
    ...
    /**
     * Map local_elediaai_questiongen exceptions to LLM-actionable tool exceptions.
     *
