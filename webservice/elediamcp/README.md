...
- **MCP prompts** — `prompts/list` and `prompts/get` expose guided Suite
  workflows and hide prompts whose tools are unavailable to the token.
...
- **OAuth 2.1 Authorization Code + PKCE** (opt-in, off by default) — compliant
  MCP clients can obtain a token through a standard browser sign-in + consent
...
## 🔐 OAuth 2.1 sign-in (optional)

An in-Moodle OAuth 2.1 Authorization Code + PKCE authorization server lets a
compliant MCP client obtain a token through a browser sign-in and consent screen,
instead of a manually created token. It is **off by default**; enable it under
the MCP configuration page (**Enable OAuth 2.1**). When enabled, the plugin
publishes Authorization Server Metadata at
`/.well-known/oauth-authorization-server` (RFC 8414) and offers Dynamic Client
Registration (RFC 7591). Only the `S256` PKCE method is accepted, clients are
public (no client secret), and redirect URIs must be pre-registered. Tokens
issued this way appear under **Preferences → MCP tokens** and can be revoked
there. Existing bearer-token clients keep working regardless of the setting.
