 ...
 *
 * Only registered when local_elediaai_selfstudy is installed. Runs as the
...
class moodle_selfstudy_create_quiz implements ai_tool {
    ...
    public static function description(): string {
        return 'Use this tool when the user asks to create, generate or start a personal '
            . 'self-study/practice quiz ("Selbstcheck", "Selbstlern-Quiz", "Übungsquiz") '
            . 'for a course topic. Do NOT call moodle_selfstudy_list_quizzes first to decide '
            . 'whether creation is possible: existing quizzes are irrelevant for creating a new '
            . 'one. Generation runs SERVER-SIDE through the Moodle site\'s AI provider — pass '
            . 'only the topic, never invent questions yourself. The quiz is private to the user '
            . '(no course activity, no question bank entry); results feed Moodle competencies as '
            . 'evidence. Requires the teacher to have enabled self-study quizzes in the course; '
            . 'a per-student daily limit applies. If the user has already asked to create/generate '
            . 'the quiz and course_id/topic are known, call this with confirm=true. Otherwise call '
            . 'once without confirm for a readiness preview, then call again with confirm=true after '
            . 'the user confirms. Afterwards use moodle_selfstudy_get_quiz to fetch the questions '
            . 'and quiz the user.';
    }
    ...
    public static function execute(array $arguments, stdClass $user): array {
        if (!class_exists('\local_elediaai_selfstudy\local\quiz_service')) {
            throw new tool_exception('local_elediaai_selfstudy is not installed on this site.');
        }
        ...
        try {
            \local_elediaai_selfstudy\local\course_settings::require_enabled_for($courseid, (int) $user->id);
            \local_elediaai_selfstudy\local\generator::require_text_provider();
        ...
        $remaining = \local_elediaai_selfstudy\local\quota::remaining((int) $user->id, $courseid);
        if ($remaining <= 0) {
        ...
        $preview = [
            ...
            'competencies_available' => count(
                \local_elediaai_selfstudy\local\competency_bridge::course_competencies($courseid)
            ),
            'grounding' => $grounding,
        ];
        ...
        try {
            $quiz = \local_elediaai_selfstudy\local\quiz_service::create(
                $courseid,
                (int) $user->id,
                $topic,
                $count,
