...
class security {
    ...
    /**
     * Whether the OAuth 2.1 Authorization Code + PKCE authorization server is enabled.
     *
     * Disabled by default: existing installations keep issuing tokens through the
     * self-service UI and internal API only, and the protected-resource metadata
     * advertises no authorization server until an administrator opts in.
     *
     * @return bool
     */
    public static function oauth_enabled(): bool {
        return (int) self::get_config('oauth_enabled', 0) === 1;
    }

    /**
     * Whether dynamic client registration (RFC 7591) is accepted.
     *
     * Only consulted when {@see self::oauth_enabled()} is true. Enabled by default
     * so zero-configuration MCP clients (Claude Desktop, Cursor, ...) can onboard.
     *
     * @return bool
     */
    public static function oauth_allow_dynamic_registration(): bool {
        return (int) self::get_config('oauth_allow_dynamic_registration', 1) === 1;
    }

    /**
     * Lifetime of an authorization code in seconds (clamped to 30..600).
     *
     * @return int
     */
    public static function oauth_code_ttl(): int {
        return max(30, min(600, (int) self::get_config('oauth_code_ttl', 300)));
    }

    /**
     * Lifetime of an issued OAuth access token in seconds; 0 means no expiry.
     *
     * @return int
     */
    public static function oauth_token_ttl(): int {
        return max(0, (int) self::get_config('oauth_token_ttl', 0));
    }
