...
- `$plugin->release` is the human-readable SemVer string and is the single
  source of truth for the advertised `serverInfo.version`: `server::get_server_version()`
  reads it from version.php at runtime, so bumping `$plugin->release` is enough
  (no separate constant to keep in sync). Update the README maturity badge too.
