...
use core_component;
use moodle_url;
use webservice_elediamcp\local\ai\source;
use stdClass;
use Throwable;
...
class moodle_get_resource implements ai_tool {
    ...
    public static function output_schema(): array {
        return [
            'type' => 'object',
            'required' => ['cmid', 'modname', 'name', 'course_id', 'url', 'content', 'summary', 'source'],
            'properties' => [
                'cmid' => ['type' => 'integer'],
                'modname' => ['type' => 'string'],
                'name' => ['type' => 'string'],
    ...
    public static function execute(array $arguments, stdClass $user): array {
        ...
        // Ready-made citation, so the consumer does not have to guess which
        // fields of this payload are quotable (see class source).
        $payload['source'] = source::build(
            $payload['name'],
            $payload['url'],
            $payload['content'] !== '' ? $payload['content'] : $payload['intro'],
            $cmid,
            $courseid
        );
