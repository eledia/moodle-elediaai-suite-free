 ...
 * - tools/list and prompts/list with pagination,
...
class server extends webservice_base_server {
    ...
    /** @var string|null Cached advertised server version; see {@see get_server_version()}. */
    ...
    public function run(): void {
        ...
        // Raw Moodle WS functions require both premium and the explicit
        // expose_raw_functions switch — the same gate that governs their
    ...
    protected function ensure_rate_limit_ok(): bool {
        $ipresult = security::enforce_rate_limit(security::rate_limit_key(null));
        if (!$ipresult['allowed']) {
            return $this->emit_rate_limit_exceeded($ipresult);
        }
        ...
        return $this->emit_rate_limit_exceeded($result);
    }

    /**
     * Emit a JSON-RPC rate limit response.
     *
     * @param array{allowed: bool, retry_after: int, remaining_minute: int, remaining_hour: int} $result
     * @return bool Always false.
     */
    protected function emit_rate_limit_exceeded(array $result): bool {
    ...
    protected function handle_mcp_method(): void {
        ...
        switch ($this->mcprequest->method) {
            ...
            case 'resources/list':
                // Resources remain reserved until their own approved slice.
                $this->send_catalog_response(['resources' => []]);
                break;
    ...
    /**
     * Advertised semantic server version for `serverInfo`/server-info.
     *
     * Derived from `$plugin->release` in version.php so the advertised MCP
     * server version is a single source of truth and can never drift from the
     * plugin release again (it silently went stale twice: stuck at 1.1.0, then
     * lagging the release once more). See docs/04-tasks.md q01.
     *
     * @return string SemVer string, e.g. "1.5.3".
     */
    public static function get_server_version(): string {
        global $CFG;
        if (self::$serverversion === null) {
            $plugin = new \stdClass();
            require($CFG->dirroot . '/webservice/elediamcp/version.php');
            self::$serverversion = (string) $plugin->release;
        }
    ...
    /**
     * Send a JSON-RPC catalogue response.
     *
     * @param array<string, mixed> $result Result payload.
     * @return void
     */
    protected function send_catalog_response(array $result): void {
        echo $this->safe_json_encode([
            'jsonrpc' => $this->mcprequest->jsonrpc,
            'id' => $this->mcprequest->id,
            'result' => $result,
    ...
    /**
     * Send a paginated prompts/list response.
     *
     * @return void
     */
    protected function send_prompts_list_response(): void {
        $cursor = null;
        if (is_array($this->mcprequest->params ?? null)) {
            $cursor = isset($this->mcprequest->params['cursor'])
                ? (string) $this->mcprequest->params['cursor'] : null;
        }
        $page = prompt_registry::paginate(prompt_registry::get_prompts((string) $this->token), $cursor);
        $result = ['prompts' => $page['items']];
        if ($page['nextCursor'] !== null) {
            $result['nextCursor'] = $page['nextCursor'];
        }
        $this->send_catalog_response($result);
    }

    /**
     * Send a prompts/get response or the appropriate JSON-RPC error.
     *
     * @return void
     */
    protected function send_prompts_get_response(): void {
        try {
            $params = is_array($this->mcprequest->params ?? null) ? $this->mcprequest->params : [];
            $name = isset($params['name']) ? (string) $params['name'] : '';
            $class = prompt_registry::find($name, (string) $this->token);
            if ($class === null) {
                throw new \InvalidArgumentException(
                    get_string('err_prompt_not_found', 'webservice_elediamcp', $name)
                );
            }
            $arguments = isset($params['arguments']) ? $params['arguments'] : [];
            if (!is_array($arguments)) {
                throw new \InvalidArgumentException(
                    get_string('err_prompt_arguments_object', 'webservice_elediamcp')
                );
            }
            global $USER;
            $arguments = prompt_registry::validate_arguments($class, $arguments);
            $this->send_catalog_response([
                'description' => $class::description(),
                'messages' => $class::render($arguments, $USER),
            ]);
        } catch (\InvalidArgumentException $exception) {
            $this->send_jsonrpc_error(-32602, $exception->getMessage());
        } catch (\Throwable $exception) {
            $this->log_throwable_for_debug($exception);
            $this->send_jsonrpc_error(-32603, get_string('err_internal_prompt_error', 'webservice_elediamcp'));
        }
    }

    /**
     * Send a JSON-RPC error for a prompt protocol operation.
     *
     * @param int $code JSON-RPC error code.
     * @param string $message Error message.
     * @return void
     */
    protected function send_jsonrpc_error(int $code, string $message): void {
        echo $this->safe_json_encode([
            'jsonrpc' => $this->mcprequest->jsonrpc ?? '2.0',
    ...
    protected function generate_error(?Exception $ex): array {
        ...
        if (debugging()) {
            $errordata['exception'] = get_class($ex);
    ...
    protected function record_tool_invocation(
        ...
        try {
            ...
            $iswrite = $this->is_write_tool($toolname, $aitool);
            if (!$iserror && $iswrite) {
            ...
            // Die Suite fuehrt ein eigenes Handlungsprotokoll (Schicht A).
            // Es wird hier gefuellt und nicht per Ereignis-Beobachter: die
            // Einstufung "schreibt" kommt aus den MCP-Annotationen des
            // Werkzeugs, und die kennt nur dieser Server. Der Kern kennt seine
            // Plugins nicht und darf nicht anfangen zu raten, welches Werkzeug
            // schreibt. Weiche Kopplung -- ohne local_elediaai_core passiert
            // schlicht nichts.
            $recorder = '\\local_elediaai_core\\local\\action_recorder';
            if (class_exists($recorder)) {
                [$contextid, $courseid] = $this->resolve_action_place($arguments);
                call_user_func([$recorder, 'record'], ...[
    ...
    /**
     * Where an action happened, derived from the arguments the agent sent.
     *
     * Most tools already take a courseid or a cmid, so the place comes for free
     * without touching a single tool. What cannot be derived stays 0 -- the
     * report then says so instead of guessing a context.
     *
     * @param array $arguments Raw tool arguments.
     * @return array{0:int,1:int} contextid, courseid
     */
    protected function resolve_action_place(array $arguments): array {
        $cmid = isset($arguments['cmid']) ? (int) $arguments['cmid'] : 0;
        if ($cmid > 0) {
            try {
                [$course, $cm] = get_course_and_cm_from_cmid($cmid);
                return [(int) \core\context\module::instance((int) $cm->id)->id, (int) $course->id];
            } catch (\Throwable $ex) {
                unset($ex);
            }
        }

        $courseid = isset($arguments['courseid']) ? (int) $arguments['courseid'] : 0;
        if ($courseid > 0) {
            try {
                return [(int) \core\context\course::instance($courseid)->id, $courseid];
            } catch (\Throwable $ex) {
                unset($ex);
            }
        }
