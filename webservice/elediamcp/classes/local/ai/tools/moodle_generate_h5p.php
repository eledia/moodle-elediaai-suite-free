...
/**
 * AI-native H5P generation tool wrapping local_elediaai_h5pauthor.
 *
 * Only registered when local_elediaai_h5pauthor is installed (see
...
class moodle_generate_h5p implements ai_tool {
    ...
    private const TYPES = ['dialogcards', 'blanks', 'singlechoice', 'multichoice'];
    ...
    public static function description(): string {
        return 'Generates interactive H5P content (dialogcards, blanks, singlechoice or multichoice) as an '
            . 'owner-bound review draft, then publishes that exact draft only after approval. '
            ...
            . 'via source_cmid. Requires local/elediaai_h5pauthor:use in the course. Two-step flow: first '
            . 'call generates and returns the actual structured preview plus draft_token; call again with '
            . 'that draft_token and confirm=true to publish it.';
    }
        ...
        return [
            ...
            'properties' => [
                ...
                'confirm' => [
                    'type' => 'boolean',
                    'default' => false,
                    'description' => 'Must be true together with draft_token to publish the reviewed draft.',
                ],
                'draft_token' => [
                    'type' => 'string',
                    'default' => '',
                    'description' => 'Owner-bound token from the generated preview. Supply with confirm=true to publish.',
                ],
            ],
        ];
    }
    ...
    public static function execute(array $arguments, stdClass $user): array {
        if (!class_exists('\local_elediaai_h5pauthor\local\authoring_service')) {
            throw new tool_exception('local_elediaai_h5pauthor is not installed on this site.');
        }
        ...
        require_capability('local/elediaai_h5pauthor:use', $coursecontext, $user->id);
        if ($target === 'activity') {
        ...
        $library = \local_elediaai_h5pauthor\local\library_checker::installed_dependency($type);
        $machinename = \local_elediaai_h5pauthor\local\content_type::machine_name($type);
        if ($library === null) {
            throw new tool_exception(
        ...
        $drafttoken = trim((string) ($arguments['draft_token'] ?? ''));
        if ($drafttoken !== '') {
            if (empty($arguments['confirm'])) {
                throw new tool_exception('confirm=true is required to publish a reviewed draft.');
            }
            try {
                $item = \local_elediaai_h5pauthor\local\authoring_service::approve($drafttoken, (int) $user->id);
            ...
            return [
                ...
                'item' => $item,
                'preview' => ['draft_token' => $drafttoken],
                'summary' => 'Published the reviewed H5P draft "' . $item['name'] . '". Link: ' . $item['url'],
            ];
        }
        ...
        try {
            $draft = \local_elediaai_h5pauthor\local\authoring_service::generate(
                $courseid,
                (int) $user->id,
                $type,
                $topic,
        ...
        $preview['draft_token'] = $draft['drafttoken'];
        $preview['generated_content'] = $draft['payload'];
        $preview['expires'] = $draft['expires'];
        return [
            'created' => false,
            'requires_confirmation' => true,
            'item' => null,
            'preview' => $preview,
            'summary' => 'Generated an unpublished H5P draft for review. Inspect generated_content, then call '
                . 'again with draft_token="' . $draft['drafttoken'] . '" and confirm=true to publish this exact draft.',
        ];
    }
    ...
    /**
     * Map local_elediaai_h5pauthor exceptions to LLM-actionable tool exceptions.
     *
