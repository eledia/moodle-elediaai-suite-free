...
final class selfstudy_tools_test extends advanced_testcase {
    ...
    /**
     * Whether local_elediaai_selfstudy is installed on the test site.
     *
     * @return bool
     */
    private static function plugin_installed(): bool {
        return class_exists('\local_elediaai_selfstudy\local\quiz_service');
    }
    ...
    public function test_execute_requires_plugin(): void {
        global $USER;
        if (self::plugin_installed()) {
            $this->markTestSkipped('local_elediaai_selfstudy is installed on this site.');
        }
    ...
    public function test_create_requires_course_opt_in(): void {
        if (!self::plugin_installed()) {
            $this->markTestSkipped('local_elediaai_selfstudy is not installed on this site.');
        }
        $this->resetAfterTest(true);
    ...
    public function test_get_and_submit_flow(): void {
        ...
        $this->getDataGenerator()->enrol_user($student->id, $course->id, 'student');
        $this->setUser($student);
        \local_elediaai_selfstudy\local\course_settings::save((int) $course->id, true, 0, 2);
