...
class tool_provider {
    ...
    public static function paginate(array $tools, ?string $cursor, ?int $pagesize = null): array {
        $page = self::paginate_items($tools, $cursor, $pagesize);
        return ['tools' => $page['items'], 'nextCursor' => $page['nextCursor']];
    }

    /**
     * Apply the shared opaque-cursor pagination to any MCP catalogue.
     *
     * @param array<int, mixed> $items
     * @param string|null $cursor
     * @param int|null $pagesize
     * @return array{items: array<int, mixed>, nextCursor: string|null}
     */
    public static function paginate_items(array $items, ?string $cursor, ?int $pagesize = null): array {
    ...
    private static function is_ai_tool_available(string $name): bool {
        // Contributed tools ship with plugins the site installed itself, so
        // they bypass the premium gating that applies to built-in tools.
        return premium::has_mcp_tools()
            || in_array($name, self::FREE_AI_TOOLS, true)
            || registry::is_contributed($name);
    }
